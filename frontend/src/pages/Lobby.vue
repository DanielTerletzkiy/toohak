...
const {startCountdown, resetCountdown} = questionStore;
...
function onQuestionChange(
  ...
  timerProgress.value = lobby.value.questionDuration;

  if (isHost.value) {
    answers.value = questionData.answers;
    progress.value = questionData.progress;
    setTimeout(() => {
...
<template>
  ...
  <d-column
      v-if="
      lobby &&
      (lobby.state === LobbyState.Idle || lobby.state === LobbyState.Started)
    "
      gap
      class="pa-6"
  >
    ...
    <d-card
        v-for="player in lobby.players as User[]"
        :key="player.socketId"
        block
  ...
  <d-column
      v-if="isHost && lobby && lobby.state !== LobbyState.Idle"
      gap
      class="pa-6"
  >
